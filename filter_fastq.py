...
def max_nargs(x):
    class RequiredLength(argparse.Action):
        def __call__(self, parser, args, values, option_string=None):
            if not len(values) <= x:
                msg = 'argument "{f}" requires maximum 2 arguments'.format(f=self.dest)
...
parser.add_argument("--gc_bounds", nargs="+", action=max_nargs(2),
                    help="range of GC percent(only minimum or minimum and maximum) USE MAXIMUM TWO VALUES!!!")
parser.add_argument("--output_base_name",
                    help="Name for output file/files")
parser.add_argument("reads",
                    help="File with reads in fastq format")
            ...
            if len(new_list[1]) < args.min_length:
                ...
                if len(args.gc_bounds) == 2:
                    ...
                    if gc_content(new_list[1]) < int(args.gc_bounds[0]):
                        Pass = False
            if not Pass and args.keep_filtered is not None:
